...
詳細画面では顔写真を登録することができます。
デイサービスは利用者の数が膨大な上、日替わりで利用されるので顔と名前を一致させるのに時間がかかってしまいます。
顔と名前を一致させるためにも、写真を登録できる機能は必要だと考え実装しました。
全員が写真を登録できるわけでもないので、写真を登録していない場合は汎用的な人物のシルエット画像が表示されるようになっています。
